function [MCNP,cellcounter,planecounter]=SSS_Bound3to1x3(MCNP,geom,cellcounter,planecounter)
...
MCNP.Boundary.Plane_String{1}=['surf ', num2str(planecounter) ' hexxc 0 0 ',num2str((ftf+.00001),'%.4f'), ' \n' ];

% MCNP.Boundary.Plane_String{1}=['surf ', num2str(planecounter) ' hexxc ', num2str(-(ftf/2),'%.4f'),' ',num2str((edgelength)/2,'%.4f '),' ',num2str((ftf+.00001),'%.4f'), ' \n' ];
