function [MCNP,cellcounter,planecounter]=SSS_Bound1to1x2by2(MCNP,geom,cellcounter,planecounter)
...
[Fuel_Hex_Cords, TieTube_Tally, hexradX, hexradY] =  CordsandSize(MCNP);
   ...
   %outside
    ...
    boundX=ftf+.000001;
    MCNP.Boundary.Plane_String{1}=['surf ', num2str(planecounter), ' cuboid ',num2str(-boundX,'%.4f'),' ',num2str(boundX,'%.4f'),' ',num2str(-(boundY),'%.4f'),' ',num2str((boundY),'%.4f'),' \n','-10 10 '];
