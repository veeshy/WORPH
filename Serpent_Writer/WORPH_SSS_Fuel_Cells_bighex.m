function [] = WORPH_SSS_Fuel_Cells_bighex(fileID,MCNP,geom)
...
TieTubespec=MCNP.TieTubeCell{1}; % be carful this assumes that you dont have poison cells in the big hex
...
if strcmp(Fuelgeom.Hex_Corner,'rounded') || strcmp(Fuelgeom.Hex_Corner,'Rounded');
    ccounter=15;
    cc=fprintf(fileID,'%i %i %.6f -%i ',Out_Cladspec.Out_Clad_Cell_Number,Out_Cladspec.Out_Clad_Cell_Number,Out_Cladspec.Density,TieTubespec.Fuel.Plane_Number);
    ...
    fprintf(fileID,'cell %i %i %i -%i  ',Out_Cladspec.Out_Clad_Cell_Number,  MCNP.FuelCell{kkk}.Universe, Out_Cladspec.Out_Clad_Cell_Number,Out_Cladspec.Rounded_Planes_Hexagon_Plane_Number,)
    fprintf(fileID,'-%i %i',TieTubespec.Fuel.Plane_Number,Out_Cladspec.Fuel_Plane_Number);
    fprintf(fileID,' & \n');
    fprintf(fileID,'imp:n=1 u=%i tmp=%.6e \n',MCNP.FuelCell{kkk}.Universe,Out_Cladspec.Temp);
