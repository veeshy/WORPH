...
Poison_load = MCNP.fuel.Poison_Frac; % atoms loaded
...
M_U = (0.0005074328*235+0.0020540448*238+0.0000053614*234+0.0000024409*236+1e-36*233+1e-36*240)*(1-Poison_load);
M_Zr = (0.0099844321*90+0.0021773630*91+0.0033281440*92+0.0033727780*94+0.0005433705*96)*(1-Poison_load);
M_C = 0.1087381552*12*(1-Poison_load);
M_P = Poison_load*M_Poison;
MCNP.Density.normal.Fuel.Fuel.Density = -(p_fuel*(1-Poison_load)+p_poison*Poison_load);


%% Write fuel composition to file if flaged

Fuel_String=...
    ['92235.80c ',num2str(0.0005074328*(1-Poison_load),'%18.10e'),' \n',...
    '92238.80c ',num2str(0.0020540448*(1-Poison_load),'%18.10e'),' \n',...
    '92234.80c ',num2str(0.0000053614*(1-Poison_load),'%18.10e'),' \n',...
    '92236.80c ',num2str(0.0000024409*(1-Poison_load),'%18.10e'),' \n',...
    '92233.80c  1e-36 \n'...
    '94240.80c  1e-36 \n' ...
    '40090.80c ',num2str(0.0099844321*(1-Poison_load),'%18.10e'),' \n',...
    '40091.80c ',num2str(0.0021773630*(1-Poison_load),'%18.10e'),' \n',...
    '40092.80c ',num2str(0.0033281440*(1-Poison_load),'%18.10e'),' \n',...
    '40094.80c ',num2str(0.0033727780*(1-Poison_load),'%18.10e'),' \n', ...
    '40096.80c ',num2str(0.0005433705*(1-Poison_load),'%18.10e'),' \n', ...
    '6000.80c ',num2str(0.1087381552*(1-Poison_load),'%18.10e'),' \n',];
...
elseif Poison_Type == 5   %Th Natural Composition0
    Poison =...
        ['90232.80c ',num2str(AD_Poison_Metal,'%18.10e'),' \n'];
...
if exist('Poison')
MCNP.Material.normal.Fuel.Material_String = strcat(Fuel_String,Poison);
else
    MCNP.Material.normal.Fuel.Material_String =Fuel_String;
end
end
